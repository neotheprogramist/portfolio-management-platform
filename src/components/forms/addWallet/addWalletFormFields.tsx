...
export default component$<AddWalletFormFieldsProps>(({ addWalletFormStore, onConnectWalletClick, isWalletConnected }) => {
  return (
    <> 
    <div>
    ...
    <label for="address" class="mb-6 flex gap-2 pb-1 text-xs text-white items-center justify-between">
        Wallet Address
         {addWalletFormStore.isExecutable ? <div>
       <button 
       ...
       >{isWalletConnected ? 'Disconnect ' : 'Connect Wallet'}</button>
         </div> : <div>
       ...
       {addWalletFormStore.isExecutable ? <div>
        {isWalletConnected ? <div class={`block w-full rounded bg-transparent border border-[#24A148] p-3 text-[#24A148] h-12 flex items-center justify-center`}> 
        wallet adddress
        {/* {addWalletFormStore.address.substring(0,6)}... */}
        </div>: <div class={`block w-full rounded bg-transparent border border-[#FDD835] p-3 text-[#FDD835] h-12 flex items-center justify-center`}> Wallet not connected</div>}
       </div> : <div class="mb-5 flex items-center gap-2">
         <input
           type="text"
           name="address"
